...
for j=d+1:D:N-d+1,
for k=d+1:D:N-d+1,
z(j,k)=(z(j+d,k+d)+z(j+d,k-d)+z(j-d,k+d)+z(j-d,k-d))/4+alpha*norminv(rand,0,1);
end;
end;
...
for j=d+1:D:N-d+1,
z(j,1)=(z(j+d,1)+z(j-d,1)+z(j,d+1))/3+alpha*norminv(rand,0,1);
z(j,N+1)=(z(j+d,N+1)+z(j-d,N+1)+z(j,N-d+1))/3+alpha*norminv(rand,0,1);
z(1,j)=(z(1,j+d)+z(1,j-d)+z(d+1,j))/3+alpha*norminv(rand,0,1);
z(N+1,j)=(z(N+1,j+d)+z(N+1,j-d)+z(N-d+1,j))/3+alpha*norminv(rand,0,1);
end;

for j=d+1:D:N-d+1,
for k=D+1:D:N-d+1,
z(j,k)=(z(j,k+d)+z(j,k-d)+z(j+d,k)+z(j-d,k))/4+alpha*norminv(rand,0,1);
end;
end;
