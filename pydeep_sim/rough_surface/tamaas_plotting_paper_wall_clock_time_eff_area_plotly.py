...
fig1 = px.scatter(
    ...
    labels={
        "eff_area": r"$A_e$ [-]",  # Plotly doesn't reliably render full LaTeX in axis titles
...
# interactive version
fig1.write_html(f"{experiment_name}_scatter_eff_area.html", include_plotlyjs="cdn")

# ---- vector export for print (recommended) ----
# pip install -U kaleido
fig1.write_image(
    f"{experiment_name}_scatter_eff_area_run_time.svg"
)  # vector, great for papers
fig1.write_image(f"{experiment_name}_scatter_eff_area_run_time.pdf")  # vector PDF
# fig1.write_image(f"{experiment_name}_scatter_eff_area_run_time.png", scale=3)  # hi-res raster fallback
