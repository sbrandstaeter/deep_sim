...
def patches_generation(H, n_iter, surf_id = 0, path_to_surface = 'surface_patches', file_tail = 'RMD', N = 128, l = 1.0, std0 = 0.09):

    assert int(N%np.sqrt(n_iter)) == 0
    resolution = int(np.log2(N/np.sqrt(n_iter)))
    print('{0} points, repeated {1} times on each side.'.format(2**resolution, int(np.sqrt(n_iter))))
    ...
    z_patch = np.block([[z_raw[i*patches_per_side+j] for j in range(patches_per_side)] for i in range(patches_per_side)])

    final_surface = 'topology_RMD_aggregated_{0:02d}x{1:03d}_{2:04d}.dat'.format(n_iter,int(N/np.sqrt(n_iter)),surf_id)
